...
**No build, no server, no dependencies.** Static HTML/CSS/JS; your data lives in the browser's local storage, with JSON export/import and optional GitHub sync. A standalone companion app to Longitude: the repo's deploy workflow copies this folder into the Pages site at `/score-study/` — it is not part of the Vite build.
